...
const UploadForm = ({ onFileChange, onSubmit, fileName = '', isLoading, error }) => {
const instructions = [
    ...
    <div className="upload-form__wrapper">
      <div className="form-instruction__wrapper">
        ...
        {instructions.map((instruction, index) => (
          <p className="form-instruction__item" key={`instruction-item-${index}`}>{`${index + 1}. ${instruction}`}</p>
        ))}
      </div>
      ...
      <p className="upload-form__hint">Accepcted file extensions: <b>.pcap</b></p>
